...
import backend.math.GeneratorAnyDistribution;
...
public class KindGenerator
{
  ...
  public static KindGenerator create(ArrayList<DistributionElement> distribution) {
    return new KindGenerator(distribution, AppInstance.defaultGeneratorName);
  }
  ...
  private KindGenerator(ArrayList<DistributionElement> distribution, String name) {
  ...
  // похоже при восстановлении вызывается он
  // TODO: момент похоже скользкий - а будет ли распределение инициализировано?
  public static Optional<KindGenerator> restoreById(Long id) {
    LoadResult<KindGenerator> q = OfyService.ofy().load().type(KindGenerator.class).id(id);
    Optional<KindGenerator> g = Optional.fromNullable(q.now());
    if (g.isPresent()) {
